    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <select className="px-3 py-2 text-sm border border-neutral-300 rounded-lg bg-white text-neutral-700 focus:outline-none focus:ring-2 focus:ring-brand-500 shadow-xs">
            <option value="last_30">Last 30 Days</option>
            <option value="last_90">Last 90 Days</option>
            <option value="year_to_date">Year to Date (2026)</option>
            <option value="custom">Custom Range...</option>
          </select>
          <button className="px-4 py-2 text-sm font-semibold text-neutral-700 bg-white border border-neutral-300 rounded-lg hover:bg-neutral-50 transition-colors shadow-xs">
            Export CSV
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 bg-white border border-neutral-200 rounded-xl shadow-xs">
          ...
          <div className="text-2xl font-bold text-neutral-900 mt-2">$86,450</div>
        ...
        <div className="p-5 bg-white border border-neutral-200 rounded-xl shadow-xs">
          ...
          <div className="text-2xl font-bold text-neutral-900 mt-2">5.53x</div>
          <div className="text-xs font-semibold text-emerald-600 mt-1 flex items-center gap-1">
        ...
        <div className="p-5 bg-white border border-neutral-200 rounded-xl shadow-xs">
          ...
          <div className="text-2xl font-bold text-neutral-900 mt-2">$2.49</div>
          <div className="text-xs font-semibold text-emerald-600 mt-1 flex items-center gap-1">
        ...
        <div className="p-5 bg-white border border-neutral-200 rounded-xl shadow-xs">
          ...
          <div className="text-2xl font-bold text-neutral-900 mt-2">6,570</div>
          <div className="text-xs font-semibold text-emerald-600 mt-1 flex items-center gap-1">
            ↑ 12.4% order volume
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs font-medium text-neutral-600">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-full bg-brand-700 inline-block"></span> Conversions
            </span>
            <span className="flex items-center gap-1 ml-3">
              <span className="w-3 h-3 rounded-full bg-neutral-300 inline-block"></span> Spend ($)
            </span>
          </div>
        </div>

        {/* Visual Chart Graphic Wireframe */}
        <div className="h-64 bg-neutral-50 rounded-lg border border-dashed border-neutral-300 flex flex-col items-center justify-center p-6 text-center">
          <div className="w-full h-full flex items-end justify-between gap-2 px-4 pt-8 pb-2">
            {[40, 65, 55, 80, 95, 70, 85, 100, 60, 75, 90, 80].map((height, i) => (
              <div key={i} className="w-full flex flex-col items-center gap-1 h-full justify-end group">
                <div 
                  className="w-full bg-brand-500 group-hover:bg-brand-600 transition-all rounded-t-sm" 
                  style={{ height: `${height}%` }}
                ></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-neutral-200 rounded-xl shadow-xs overflow-hidden">
            ...
            <table className="w-full text-left text-sm text-neutral-600">
              ...
              <tbody className="divide-y divide-neutral-200">
                {CHANNEL_DATA.map((item) => (
                  <tr key={item.channel} className="hover:bg-neutral-50/60 transition-colors">
                    ...
                    <td className="px-5 py-4 font-medium text-neutral-900">{item.cpa}</td>
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-xs space-y-4">
          ...
          <div className="space-y-3">
            {TOP_CAMPAIGNS.map((campaign, index) => (
              <div
                key={campaign.id}
                className="p-3 rounded-lg border border-neutral-100 bg-neutral-50/50 flex items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded-full bg-neutral-200 text-neutral-700 text-xs font-bold flex items-center justify-center">
                    #{index + 1}
                  </span>
                  <div>
                    <Link
                      href={`/campaigns/${campaign.id}`}
                      className="text-xs font-semibold text-neutral-900 hover:text-brand-700 transition-colors line-clamp-1"
                    >
                      {campaign.name}
                    </Link>
          ...
          <Link
            href="/campaigns"
            className="block text-center text-xs font-semibold text-brand-700 hover:text-brand-600 pt-2"
          >
